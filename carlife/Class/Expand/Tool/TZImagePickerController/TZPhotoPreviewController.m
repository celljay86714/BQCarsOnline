...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UIImage imageNamedFromMyBundle:@"back_topbar_noamal"] forState:UIControlStateNormal];
    [_backButton setImage:[UIImage imageNamedFromMyBundle:@"back_topbar_pressed"] forState:UIControlStateHighlighted];
...
- (void)configBottomToolBar {
    ...
    CGRect labelFrame = CGRectMake(self.view.tz_width - 56 - labelWidth, 7, labelWidth, 30);
    ...
    /*
     ...
     _numberImageView.frame = labelFrame;
     _numberImageView.hidden = _tzImagePickerVc.selectedModels.count <= 0;
     */
    _numberLable = [[UILabel alloc] init];
    _numberLable.frame = labelFrame;
    _numberLable.font = [UIFont systemFontOfSize:15];
    _numberLable.textColor = VKDefaultYellow;
    _numberLable.textAlignment = NSTextAlignmentCenter;
    _numberLable.text = [NSString stringWithFormat:@"(%zd/%zd)",_tzImagePickerVc.selectedModels.count, _tzImagePickerVc.maxImagesCount];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell.singleTapGestureBlock) {
        ...
        cell.singleTapGestureBlock = ^(){
            ...
            //            self.navigationController.navigationBar.hidden = _weakIsHideNaviBar;
            [self.navigationController setNavigationBarHidden:_weakIsHideNaviBar];
...
- (void)logPhotoBytes {
    ...
    [_models enumerateObjectsUsingBlock:^(TZAssetModel  *obj, NSUInteger idx, BOOL * stop) {
        if (obj.isSelected) {
            
            [[TZImageManager manager] getPhotosBytesWithArray:@[obj] completion:^(NSString *totalBytes) {
                
                DDLogInfo(@"图片大小 %@", totalBytes);
            }];
        }
    }];
}
